...
class TabbedTranslationMixin:
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        for field in self.get_fields(request, obj):
            ...
            elif all(not field.endswith(lang) for lang in settings.LANGUAGES):
                non_translated_fields.append(field)
        fieldsets = tuple()
        if non_translated_fields:
            fieldsets += (
                (_('General'), {'fields': non_translated_fields}),
            )
        for language in settings.LANGUAGES:
            fieldsets += (
                (language[1], {'fields': list(map(lambda x: x + '_' + language[0], translated_fields))}),
            )
        return fieldsets
